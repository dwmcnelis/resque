...
class Worker implements LoggerAwareInterface
{
	...
	{
		if (method_exists($job, 'beforePerform')) {
			try {
				$job->beforePerform();
			} catch (Exception $e) {
		...
		if (method_exists($job, 'afterPerform')) {
			try {
				$job->afterPerform();
			} catch (Exception $e) {
	...
	public function reserve()
	{
		...
		foreach ($this->queues as $queue) {
			...
			try {
				...
				$this->logger->notice('Job class {class} failed because {error}', array(
					'class' => $payload['class'],
	...
	public function unregister()
	{
		...
		if ($this->currentJob) {
			$this->logger->notice('Lost job on shutdown: {class}', array(
				'class' => $this->currentJob->getPayload()['class']
			));
